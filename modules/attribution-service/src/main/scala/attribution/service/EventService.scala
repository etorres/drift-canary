...
trait EventService:
  def addIfAbsent(event: Event): IO[Unit]
  def find(
...
object EventService:
  def inMemory(using logger: StructuredLogger[IO]): IO[EventService] =
    IO.delay:
      ...
      InMemory(concurrentHashMap, mapRef)
  ...
  final class InMemory(
      ...
      extends BaseService[Event](mapRef)
      with EventService:
